...
public:
    ...
    unsigned long int merge(const ParetoFrontier &frontier);

    // Merge pareto frontier solutions with shift
    unsigned long int merge(const ParetoFrontier &frontier, const ObjType *shift);

    // Convolute two nodes from this set to this one
    void convolute(const ParetoFrontier &fA, const ParetoFrontier &fB, MultiObjectiveStats *stats);
...
//
inline unsigned long int ParetoFrontier::merge(const ParetoFrontier &frontier)
{
    // count the number of comparisons
    unsigned long int num_comparisons;
...
//
inline unsigned long int ParetoFrontier::merge(const ParetoFrontier &frontier, const ObjType *shift)
{
    // count the number of comparisons
    unsigned long int num_comparisons;
    // last position to check
...
// Convolute two nodes from this set to this one
//
inline void ParetoFrontier::convolute(const ParetoFrontier &fA, const ParetoFrontier &fB, MultiObjectiveStats *stats)
{
    if (fA.sols.size() < fB.sols.size())
    {
        for (int j = 0; j < fA.sols.size(); j += NOBJS)
        {
            ...
            stats->num_comparisons += merge(fB, auxB);
        }
    }
    else
    {
        for (int j = 0; j < fB.sols.size(); j += NOBJS)
        {
            ...
            stats->num_comparisons += merge(fA, auxB);
        }
    }
}
