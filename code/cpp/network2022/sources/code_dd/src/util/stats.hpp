...
struct MultiObjectiveStats
{
    ...
    int layer_coupling;
    // Number of Pareto dominance checks conducted to enumerate the Pareto frontier
    unsigned long int num_comparisons;

    vector<unsigned long int> total_sol_per_layer_topdown;
    vector<unsigned long int> total_sol_per_layer_bottomup;
    unsigned long int total_sol;

    // Constructor
    MultiObjectiveStats()
        : pareto_dominance_time(0), pareto_dominance_filtered(0), layer_coupling(0), num_comparisons(0), total_sol(0)
    {
    }

    void print_sol_per_layer(vector<unsigned long int> sol_per_layer)
    {
        cout << "# ";
        for (int i; i < sol_per_layer.size(); ++i)
        {
            cout << sol_per_layer[i] << " ";
        }
    }
};
